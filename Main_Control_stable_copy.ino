...
#define max_wheel 60 // maximum speed for driving wheels
#define BDCM_speed 60 // max deviation from 90 for BDCM Servo
#define thresh 12 // "close enough" for flipper position target about 1 degree
...
#define extra_dead 100 // mostly for right joystick to separate roll and pitch
...
void setup() {
  ...
  /*
    // move both directions to make sure it works
    BDCM.write(110);
    delay(500);
    BDCM.write(70);
    delay(500);
  ...
  if (Serial.available() > 8) {
    ...
    if ((abs(incoming[1]) > deadband) || (abs(incoming[2]) > deadband)) {
      ...
      if (freewater_state) {
        ...
        /*
          Serial.print("Left x: ");
          Serial.print(temp.x_dir, DEC);
          Serial.print(", y: ");
          Serial.println(temp.y_dir, DEC);
        */
    ...
    //    if (((abs(incoming[3]) > deadband) || (abs(incoming[4]) > deadband)) && (freewater_state)) {
    //      unsigned int enc_dir = convert_joy_to_enc(incoming[3], incoming[4]);
    //      if (!wiggle_state) {
    //        Serial.println("Begin roll/pitch motion.");
    //        // if not already performing rotations, put flippers in appropriate locations
    //        PID_set_pos_one(1, enc_dir);
    //        PID_set_pos_one(2, enc_dir);
    //        PID_set_pos_one(3, enc_dir);
    //        // then move everything in the forward direction and set motor state direction all to forward
    //        cmd_pololu(1, 1, 30);
    //        cmd_pololu(2, 1, 30);
    //        cmd_pololu(3, 1, 30);
    //        m1_state = true;
    ...
    //        m3_state = true;
    //      }
    //      unsigned int enc_dirs[] = {enc_dir, enc_dir , enc_dir};
    //      unsigned char powers[] = {30, 30, 30};
    //      wiggle_state = true;
    //      all_oscillate_live(enc_dir, powers);
    //      // horizontal of right joystick used to adjust roll
    //      boolean dir;
    //      if (incoming[3] > 0) {
    //        dir = true;
...
boolean choppy_set_pos_one (unsigned char m_box, unsigned int target) {
  ...
  */
  ...
  while (!(m1_reach)) {
    cmd_pololu(m_box, 0, 0); // stop motor
    delay(motor_stop_delay); // wait for wire interference to stop
    ...
    */
    ...
    if (motor_cmds.dist < slow_down_range) {
      close_speed = flip_speed - (float) motor_cmds.dist / 6;
...
boolean PID_set_pos_one (unsigned char motor_num, unsigned int target) {
  ...
  */
  ...
  while (displacement.dist > thresh) {
    ...
    unsigned int speedy = displacement.dist * 0.3;
    if (speedy <= 15) {
      speedy = 15;
    }
